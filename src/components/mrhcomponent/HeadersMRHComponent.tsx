...
const HeadersMRHComponent: React.FC = () => {
  ...
  const [restoreButtonText, setRestoreButtonText] =
  ...
  useEffect(() => {
    if (!justMounted) {
      setHeadersMRH(
        headersMRH.filter((header) => header.name !== "" || header.value !== "")
      );
    }
  ...
  return (
    <>
      <div className="mrh-label">
        Headers are auto-saved, add/change them as needed.
      </div>
      <div className="mrh-label flex-icon">
        <img
      ...
      <div className="mrh-label flex-icon">
        ¯\_(ツ)_/¯ Don't worry if you screw up, you can always restore the
        default headers.
      </div>
      <div className="mrh-label">
        You currently have{" "}
        <span className="squared-number">{headersMRH.length}</span> headers
        {headersMRH.length == 0
          ? ", add one by clicking on 'Add Header' button"
          : ""}
      </div>
        ...
        {headersMRH.map((header, index) => (
          <div className="table-row" key={index}>
            ...
            <div className="table-cell buttons-div">
              ...
              >
                <Clipboard
                  className="icon-copy-header-value"
                  data-tooltip="Copy Value"
                  onClick={() => copyHeaderVal(index)}
                />
                <Trash
                  className="icon-delete-header"
      ...
      <div className="btnContainerMRH">
        ...
        <button
          ...
          style={{
            backgroundColor: `${restoreButtonBGColor}`,
            color: `${btnTextColor}`,
          }}
