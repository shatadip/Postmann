...
const VariablesComponent: React.FC = () => {
  ...
  const handleKeyPress = (
    ...
    if (e.key === "Tab") {
      ...
      if (e.shiftKey) {
        ...
        if (key === "name") {
          // If currently focused on a name field
          ...
          // If currently focused on a value field
          // Delay the focus change after the blur event
          setTimeout(() => {
        ...
        if (key === "name") {
          ...
          // If currently focused on a value field and not the last variable
  ...
  */}
      ...
      {variables.length > 0 && (
        <>
          <div className="vars-label flex-icon">
            <img
              src="click-outside-2.png"
              alt="Click Outside"
              style={{ width: "24px" }}
            />{" "}
            Click outside to save
          </div>
          <div className="vars-label flex-icon" style={{ marginLeft: "6px" }}>
            <img src="tab.png" alt="Tab" style={{ width: "17px" }} /> Hit
      ...
      <div className="vars-label">
        You currently have{" "}
        <span className="squared-number">{variables.length}</span>{" "}
        {variables.length == 1 ? "variable" : "variables"}
        {variables.length == 0
          ? ", create one by clicking on 'Add Variable' button."
          : ""}
      </div>
      <div className="table-container">
        {variables.length > 0 && (
          <div className="table-row header goldenrod">
        ...
        {variables.map((variable, index) => (
          <div className="table-row" key={index}>
            <div
              ref={(element) =>
                (nameRefs.current[index] = element as HTMLDivElement)
              }
            ...
            <div
              ref={(element) =>
                (valueRefs.current[index] = element as HTMLDivElement)
              } // Ref for value field
            ...
            <div className="table-cell buttons-div">
              ...
              >
                <Clipboard
                  className="icon-copy-variable-value"
                  data-tooltip="Copy Value"
                  onClick={() => copyVarVal(index)}
                />
                <Trash
                  className="icon-delete-variable"
      ...
      <div className="buttonContainerVars">
        ...
        {variables.length > 0 && (
          <button onClick={deleteAllVariables} id="btnDelAllVars">
            <ExclamationTriangle />
      ...
      <div
        className="vars-label max-num-warn"
        style={{ display: `${warnStyle}` }}
      >
        Max {maxNumOfVars} Variables, delete some old vars to add more.
      </div>
    </>
  );
};
